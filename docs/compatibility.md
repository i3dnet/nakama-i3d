...
Session storage retains the existing top-level InstanceInfo shape and adds private _i3d allocation/admission state. Provider refreshes own address, status and ordinary metadata; local capacity and admitted-player estimates survive refresh. New allocations reset local state for reused instance IDs. Mutations use Nakama 3.41's StorageWriteRetry with five conflict retries and versioned writes; ordinary storage errors are not retried.

Join provides limited local admission accounting. Duplicate users are idempotent within the current admission epoch, and a group can be partially admitted when capacity is insufficient. Returned SessionId values are empty: these are not provider-issued reservations, do not expire after one minute, and are not game-server authentication tokens. An authoritative Update player_count report replaces the estimate and resets local deduplication; the payload does not identify individual connected players. Applications must handle reconnection, game-server validation and abandoned admissions explicitly. This adapter does not claim the full expiring-reservation contract described by Nakama's generic FleetManager interface.
