...
sh scripts/check-plugin.sh builds and loads the normal plugin in Nakama 3.41.0 and tests lifecycle RPC authentication. python3 scripts/check-smoke-cleanup.py exercises failure cleanup with fake tools and never starts containers. sh scripts/smoke.sh runs the full scenario with Docker Compose, Python 3 and Go installed. The two shell scripts each create their own Compose project with random localhost ports and a temporary PostgreSQL database, and clean up only that project.

The smoke script authenticates two clients, opens real sockets and submits one ticket each. It checks one allocation, metadata and connection notifications, storage before notification, forced native StorageWriteRetry conflicts, concurrent Join capacity, indexed ordering by player count and descending creation time across single-record pages, rejected player RPCs, trusted lifecycle update/restart, recovery after a missed update over multiple provider pages, invalid readiness, one cleanup restart for a confirmed failed allocation, and allocation timeout without blind retry. Unit tests deterministically cover late/duplicate completion and race boundaries.
...
The provider mock accepts quoted equality filters joined by and, including escaped quotes and conjunctions inside values. Allocation metadata merges by key; explicit null deletes a key and an empty string remains a value. Pagination tokens retain an immutable filtered result snapshot, ignore replacement filters and expire after one minute. These cursors are test-service behavior, not a persistence guarantee for production One API.
